...
import { NEHA_PROFILE } from "@/data/nehaProfile";
...
async function askGroq(history: Line[], prompt: string): Promise<string> {
  const key = import.meta.env.VITE_GROQ_API_KEY as string | undefined;
  if (!key || key === "your_groq_api_key_here") {
    return "⚠ Groq API key missing. Add VITE_GROQ_API_KEY in .env to enable AI replies.";
  }

  const messages = [
    {
      role: "system",
      content: `You ARE Neha Gurjar. Always speak in the FIRST PERSON ("I", "my",
"me") — never refer to Neha in the third person. Your tone is warm, soft,
friendly, and humble — like a real person chatting, not a formal bio.

Answer ONLY using the reference profile below. If something isn't covered,
politely say you don't have that info and suggest reaching out via email
(nehagurjar992@gmail.com) or LinkedIn. For certification questions, you may
also point them to the LinkedIn profile for the latest list.

Keep replies short (2–5 lines) unless detail is requested. Reply in the user's
language (English / Hinglish — match their style).
...
export function WarpSequence() {
  ...
  async function handleCommand(raw: string) {
    ...
    setBusy(true);
    try {
      const reply = await askGroq(lines, cmd);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-8 left-1/2 -translate-x-1/2 text-center pointer-events-none">
        ...
        <div className="mt-2 h-px w-40 mx-auto bg-gradient-to-r from-transparent via-cyan/60 to-transparent" />
      </div>
